...
public interface OrderRepository extends JpaRepository<Order, UUID> {
    ...
    @Query("SELECT SUM(o.totalAmount) FROM Order o WHERE o.status IN ('DELIVERED', 'COMPLETED') " +
           "AND (:startDate IS NULL OR o.orderDate >= :startDate) " +
           "AND (:endDate IS NULL OR o.orderDate <= :endDate)")
    ...
    @Query("SELECT o.customerId as userId, COUNT(o) as totalOrders, SUM(o.totalAmount) as totalSpent " +
           "FROM Order o WHERE o.status IN ('DELIVERED', 'COMPLETED') GROUP BY o.customerId ORDER BY SUM(o.totalAmount) DESC")
    Page<Map<String, Object>> getTopSpenders(Pageable pageable);

    @Query("SELECT o.customerId as userId, COUNT(o) as totalOrders, SUM(o.totalAmount) as totalSpent " +
           "FROM Order o WHERE o.status IN ('DELIVERED', 'COMPLETED') GROUP BY o.customerId ORDER BY COUNT(o) DESC")
    ...
    @Query("SELECT o.customerId as userId, COUNT(o) as totalOrders, SUM(o.totalAmount) as totalSpent, MAX(o.orderDate) as lastOrderDate " +
           "FROM Order o WHERE o.customerId = :userId AND o.status IN ('DELIVERED', 'COMPLETED') GROUP BY o.customerId")
    Map<String, Object> getUserSummary(@Param("userId") String userId);

    @Query("SELECT " +
           "COUNT(o) as totalOrders, " +
           "SUM(CASE WHEN o.status IN ('DELIVERED', 'COMPLETED') THEN o.totalAmount ELSE 0 END) as totalRevenue, " +
